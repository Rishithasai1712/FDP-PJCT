...
import front6  # For manual entry (live prediction module)
import v1  # For visualizations
import csv_utils  # Rename this to avoid conflict with the built-in 'csv' module
import joblib
...
if st.session_state.page == 'home':
    ...
    st.image("img/home.jpg", use_container_width=True)
    ...
    # "Getting Started" Page with two buttons
    st.title("Getting Started with Fraud Detection")

    # Add a brief description for the Getting Started page
    st.markdown(
        "Welcome to the **Fraud Detection** system! You can start by either uploading a file with multiple transactions "
        "or manually entering a single transaction to predict whether it is fraudulent or not."
    )
    ...
    with col1:
        if st.button("📁 Upload File"):
            st.session_state.page = 'bulk_check'  # Redirect to bulk file upload (csv_utils.render_bulk_check)
    
    with col2:
        if st.button("✍️ Manual Entry"):
            st.session_state.page = 'manual_entry'  # Redirect to live prediction (front6.main)

elif st.session_state.page == 'bulk_check':
    # File upload widget to let the user upload a CSV file
    uploaded_file = st.file_uploader("Upload a CSV file for bulk processing", type=["csv"])

    # Call render_bulk_check only if a file is uploaded
    if uploaded_file is not None:
        csv_utils.render_bulk_check(uploaded_file)  # Pass the uploaded file to the function
    else:
        st.warning("Please upload a CSV file.")

elif st.session_state.page == 'manual_entry':
    front6.main()  # Handles live transaction entry for fraud detection
